...
**Tester:** `tests/test_planner_llm.py` — mapping NO/EN, `noop`→LLM med mock, eksakt regel uten LLM-kall, ugyldig/lav confidence LLM → `noop` + status, `planner_mode` **`noop`** fra regler. Øvrige planner-tester (`test_planner_*`) bekrefter at **eksakte/regex**-triggere fortsatt fungerer. Kjør: `python -m unittest discover -s tests -p 'test*.py'`.
