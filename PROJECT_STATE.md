...
**LLM-router — guardrail mot faktura-/betalings-falske positive (2026-03-21, runde 5):** **`_standalone_green_request`** / **`_non_green_accounting_context`** / **`_heuristic_blocked`**: faktura-/betalings-intent (`_classify_intent`), prosjekt+kunde, lønn/payroll, måneds-/periodisavslutning, purring/gebyr+ faktura, **blokkerer** heuristisk grønn score og **forkaster** LLM-valg av grønt workflow når konteksten ikke er en **selvstendig** CRM-/vare-/ansatt-oppgave. **`try_llm_plan_after_noop_with_detail`:** ved avvisning → **`guardrail_rejected_llm_green`** / **`planner_llm_status`:** **`guardrail_rejected_green`**. **`heuristic_green_workflow_after_llm_noop`:** lavere terskel (**`_HEURISTIC_MIN_SCORE_STANDALONE`**) når **`_standalone_green_request`** er sann (flere `llm_noop`-overrides på tydelig grønt). Tester: **`tests/test_planner_llm_oos_guardrails.py`**.
